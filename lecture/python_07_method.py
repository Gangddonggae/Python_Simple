...
#  ex) print(), len(), tupe(), format(), list(), tuple()
#
...
#   - paramter로 int, str, float, bool, list 등 사용 가능
#   - 심지어 사용자 정의 함수를 parameter로 전달 가능
#   - paramtert값 2개 이상 사용시 정의된 순서대로 전달해야함


def sub_two_value(x: int,y: int): #타입 hint
    n = x-y
    return n
a, b = 15, 20
...
#def test(a, b, c=3):
#def test(a, b=2, c=3): #셋다 가능 값 안받으면 저 값 대입
#def test(a=1, b=2, c=3):
#def test(a=1, b, c):
#def test(a, b=2, c):  # 셋다 불가능 오른쪽에서 부터
#def test(a=1, b=2, c):
...
def test(num1, num2):
    print(num1,num2)
    return      #지역변수 이므로 함수에서 나오면 값 없어짐
test(30,40)
print(num1, num2)
...
#   - 전달되는 paratmeter의 개수가 고정적이지 않은 경우
#   - print() format()
